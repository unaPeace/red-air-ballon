...
function generateTree() {
  ...
  const treeColors = ["#4FC700", "#459212", "#0DC942"];
  const color = treeColors[Math.floor(Math.random() * 3)];
...
function animate() {
  ...
  const velocityChangeWhileCooling = 0.1;
...
function drawHeader() {
  ...
  ctx.fillStyle = fuel <= 30 ? "rgba(255,0,0,0.5)" : "rgba(90,255,0,0.5)";
  ctx.fillRect(30, 30, (150 * fuel) / 100, 30);

  // Score
  const score = Math.floor(balloonX / 10);
  ctx.fillStyle = "black";
...
function drawSky() {
  ...
  gradient.addColorStop(0, "#0D83F9");
